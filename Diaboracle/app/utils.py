...
def is_date_in_intervals(date_to_check, date_list):
    ...
    if isinstance(date_to_check, str):
        date_to_check = datetime.datetime.strptime(date_to_check, '%Y-%m-%d').date()
    ...
    date_list = [
        datetime.datetime.strptime(date, '%Y-%m-%d').date() if isinstance(date, str) else date
    ...
    for i in range(len(date_list) - 1):
        if date_list[i] <= date_to_check <= date_list[i + 1]:
            return True, date_list[i].strftime('%Y-%m-%d')
